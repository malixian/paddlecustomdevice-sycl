...
def create_paddle_case(op_type, callback):
    class PaddleCls(unittest.TestCase):
        def setUp(self):
            ...
            self.place = base.CustomPlace("SYCL", 0)
            if core.is_compiled_with_cuda():
                self.place = paddle.CustomPlace("SYCL", 0)
        ...
        def test_dynamic_api(self):
            paddle.disable_static(paddle.CustomPlace("SYCL", 0))
    ...
    def test_errors(self):
        ...
        with program_guard(Program(), Program()):
            ...
            y = base.create_lod_tensor(
                numpy.array([[-1]]), [[1]], base.CustomPlace("SYCL", 0)
            )
...
class TestCompareOpPlace(unittest.TestCase):
    ...
    def test_place_2(self):
        ...
        if core.is_compiled_with_cuda():
            place = paddle.CustomPlace("SYCL", 0)
            data_place = paddle.CustomPlace("SYCL", 0)
