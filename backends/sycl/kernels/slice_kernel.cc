...
void SliceRawKernel(const phi::Context& ctx,
  ...
  show_kernel("slice kernel in_dim = "<<in_dims<<" ; out_dims = "<<out_dims<<" axes size = "<<axes.size()<<" 0 = "<<axes[0]<<" start = "<<starts<<" end = "<<ends)
  ...
  if(in_dims.size()==1){
          int num_of_element = ends[0]-starts[0];
	  auto out_data = ctx.template Alloc<T>(out);
	  out->Resize(out_dims);
	  //std::cout<<"++++++++++++++++ "<<*(input.data<T>())<<std::endl;
	  memcpy(out_data,input.data<T>()+starts[0],num_of_element*sizeof(T));
	  return;
  }
  
  std::cout<<"slice dims = {";
  for(auto temp_dim:slice_dims){
	  std::cout<<temp_dim<<",";
  }
  std::cout<<"} out_dim = {";
  for(auto temp_dim:out_dims){
          std::cout<<temp_dim<<",";
  }
  std::cout<<"}"<<std::endl;
  ...
  for (auto i = 0; i < numel; i += slice_dims.back()) {
	  std::cout<<"numel = "<<numel<<"i = "<<i<<std::endl;
    memcpy(out_data + i,
