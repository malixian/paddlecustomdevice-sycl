...
void FullKernel(const phi::Context& dev_ctx,
  ...
  if(!out->initialized()){
          std::cout<<"kernel begin initialized()"<<std::endl;
  }
  show_kernel(
       "Full-ONEDNN type=" << dnn_support::type2String<T>::name()<<" shape= "<<shape.GetData()<<" out dims:"<<out->dims()<<" full val:"<<val.to<T>());
      //  "Full-ONEDNN type=" << dnn_support::type2String<T>::name()<<" shape= "<<shape.GetData()<<" full val:"<<val.to<T>());

  auto int_shape = shape.GetData();
  //if(int_shape.size() == 0)return ;
  std::cout<<"int_shape = "<<std::endl;
  for(int temp_dim:int_shape){
