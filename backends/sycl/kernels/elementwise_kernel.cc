...
void MultiplyRawKernelGPU(const phi::Context& dev_ctx,
                          ...
                          int axis,
                          phi::DenseTensor* out) {
  show_kernel("ElementWise-SYCL-MUL type=" << dnn_support::type2String<T>::name()<<" xdim = "<<x.dims()<<" ydim = "<<y.dims());
...
void MultiplyOneDNNRawKernel(const phi::Context& dev_ctx,
                             ...
                             phi::DenseTensor* out) {
  show_kernel(
      "ElementWise-ONEDNN type=" << dnn_support::type2String<T>::name()<<" xdim = "<<x.dims()<<" ydim = "<<y.dims()<<"axis = "<<axis);
  ...
  engine_stream.wait();
  std::cout<<"elementwise end!!!!"<<std::endl;
}
...
void MultiplyMainRaw(const phi::Context& dev_ctx,
                     ...
                     int axis,
                     phi::DenseTensor* out) {
  show_kernel("Elementwise Kernel"<<" xdim = "<<x.dims()<<" ydim = "<<y.dims());
