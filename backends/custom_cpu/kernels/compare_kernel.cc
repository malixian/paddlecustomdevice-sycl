...
void NotEqualRawKernel(const phi::Context& dev_ctx,
  ...
  cout<<"NotEqualRawKernel x_dim = {";
  for(auto temp_dim:x_dims)cout<<temp_dim<<",";
  cout<<"} y_dim = {";
  for(auto temp_dim:y_dims)cout<<temp_dim<<",";
  cout<<"} dst_dim = {";
  for(auto temp_dim:dst_dims)cout<<temp_dim<<",";
  cout<<endl;
...
void GreaterThanRawKernel(const phi::Context& dev_ctx,
  ...
  cout<<"GreaterThanRawKernel x_dim = {";
  for(auto temp_dim:x_dims)cout<<temp_dim<<",";
  cout<<"} y_dim = {";
  for(auto temp_dim:y_dims)cout<<temp_dim<<",";
  cout<<"} dst_dim = {";
  ...
  auto y_data = tmp_y.data<T>();
  auto out_data = dev_ctx.template Alloc<bool>(out);
  cout<<"} inputx = "<<x_data<<" ;inputy = "<<y_data <<" ;output = "<<out_data<<endl;
  auto numel = out->numel();
  for (auto i = 0; i < numel; ++i) {
    out_data[i] = x_data[i] > y_data[i];
  }
...
void GreaterEqualRawKernel(const phi::Context& dev_ctx,
  ...
  cout<<"GreaterEqualRawKernel x_dim = {";
  for(auto temp_dim:x_dims)cout<<temp_dim<<",";
  cout<<"} y_dim = {";
  for(auto temp_dim:y_dims)cout<<temp_dim<<",";
  cout<<"} dst_dim = {";
